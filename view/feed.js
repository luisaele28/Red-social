...
import {createpost, showPosts} from '../lib/firebase.js';
...
const generatePosts = (posts) => {
    let htmlPost="";
        posts.forEach((post)=>{
           const showEdit=showButtonsEditPost(post);
            htmlPost+=`
                <div id="postDone" class="postPublished">
                    <div id="tittlePostPublished" class="tittle-post-published">
                        <div class="user" >
                            ...
                            ${
                                showEdit ? '' : `
                                <div id="buttonUserEdit-${post.id}" class="buttonUserEdit ">
                                    <button  class="editPublished edit" data-post="${post.id}">
                                        <svg data-post="${post.id}" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                        ...
                        <div id="buttonSaveCancel-${post.id}" class="hide">
                            <button id="save" class="save" data-post="${post.id}">
                                <svg data-post="${post.id}" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5" />
                                 </svg>                          
                            </button>
                            <button id="cancel" class="cancel" data-post="${post.id}" >
                                <svg data-post="${post.id}" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                    ...
                    <p id="inputPost-${post.id}" class="inputPost">${post.description}</p>
                    <textarea id="textareaEdit-${post.id}" class="hide textareaEdit" rows="4"> ${post.description}</textarea>
...
function showButtonsEditPost (p) {
